...
This extension is mainly a low-level helper extension for other extensions, and provides some tools for authors who want to make specific customizations to the story. It isn't directly useful to most other authors.
...
Similarly to other phrases, these too target only the last element with the name. To target all elements of that name:
...
In the grand tradition of hitting a nail with a wrecking ball, this example creates a "carousel" of a selection of Shakespeare's sonnets. Everything else is removed, including the command prompt, except two links that show next and previous sonnets. When the links are clicked, the previously shown sonnet is hidden and the next one is show instead.
